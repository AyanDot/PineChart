...
| Nested `if`/`else` | ✅ | Tested 2+ levels deep |
| `if` as expression | ✅ | Returns value from last line of block |
| `switch` statement | ✅ | Switch with cases and default |
...
| `for i = start to end by step` | ✅ | Range with step |
...
| `varip` declaration | ✅ | Intrabar persistent |
